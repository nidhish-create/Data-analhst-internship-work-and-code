...
#                       where
#                         direction = 'inbound'
#                         and date(call_created_on) >= '{start}' 
   ...
   #                """,
    ...
    "ayuMitra": """select email as ayuMitraEmail , phone ,
                    ...
                    where
                        ...
                        """,
    "agents": '''select email , phone as phoneNumber,
                    case when pcp.cityName = 'Chandigarh' then 'CHD'
                         when pcp.cityName = 'Bangalore' then 'BLR'
                         when pcp.cityName is not null then pcp.cityName 
                         else 'No City' end as city
                from ayu_personnel_details csd
                left join ayu_cities pcp on csd.cityId = pcp.id
                where 
                personnelType = 'CUSTOMER_SUPPORT'
                and     phone is not null and phone != ''   
    '''
...
def fetch_data(conn, callsids):
    fetched_val = {} 
    for lookup, query in queries.items():
        print(lookup) 
        # if lookup == 'exotel':
        #     fetched_val[lookup] = fetch_record(conn, query.format(start = yesterday , end = yesterday))     
        #     callsids = ''
        #     for x in fetched_val[lookup]:
        #         callsids += "'"+x['sid']+"',"
        if lookup in ('exotelInsights','exotelCallInsights'):
...
def getagents_info(leg1AgentNumber,leg2AgentNumber,leg3AgentNumber,agentsdump_,ayuPersonnelType):
    ...
    if ayuPersonnelType == 'AYU_MITRA':
        agentsdump = agentsdump_['AYU_MITRA']
    else:
        agentsdump = agentsdump_['CS']
...
def upload_to_bq(exotel_summary_bq):
    ...
    table_ref = client.dataset("tables_crons").table("exotel_summary")
...
def lambda_handler(event, context):
    try:
        ...
        dataSets = fetch_data(read_connection_obj, callsids)
        ...
        agentsdump_ = {'CS': {}, 'AYU_MITRA': {}}
        # agentsdump = {}
        for index,val in agents.iterrows():
            if val['agent_no'] not in agentsdump_['CS'].keys():
                agentsdump_['CS'][val['agent_no']] = {}
                agentsdump_['CS'][val['agent_no']]['email'] = val['email']
                agentsdump_['CS'][val['agent_no']]['city'] = val['city']
            
                
        for index,val in ayuMitra.iterrows():
            if val['agent_no'] not in agentsdump_['AYU_MITRA'].keys():
                agentsdump_['AYU_MITRA'][val['agent_no']] = {}
                agentsdump_['AYU_MITRA'][val['agent_no']]['email'] = val['ayuMitraEmail']
                agentsdump_['AYU_MITRA'][val['agent_no']]['city'] = val['city']
        ...
        df3 = exotel.apply(lambda x: getagents_info(x['leg1AgentNumber'],x['leg2AgentNumber'],x['leg3AgentNumber'],agentsdump_,x['ayuPersonnelType']),axis=1, result_type='expand')
        ...
        Subject = 'Exotel Inbound Summary To BQ error'
