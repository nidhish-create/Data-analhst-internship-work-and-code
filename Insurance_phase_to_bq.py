...
queries = {"Insurance_phase_to_bq": '''select * from insurance_phase
'''
...
def upload_to_BQ(agent):
    ...
    table_ref = client.dataset("tables_crons").table("Insurance_phase")
    
    job_config = bigquery.LoadJobConfig(
        schema=[
            bigquery.SchemaField('phaseId', 'INTEGER'),
            bigquery.SchemaField('phaseType', 'STRING'),
            bigquery.SchemaField('insuranceCaseId', 'INTEGER'),
            bigquery.SchemaField('status', 'STRING'),
            bigquery.SchemaField('reason', 'STRING'),
            bigquery.SchemaField('assignedTo', 'INTEGER'),
...
def lambda_handler(event, context):  
    ...
    try:
        ...
        Insurance_phase_to_bq = pd.DataFrame(dataSets['Insurance_phase_to_bq'])
 
        upload_to_BQ(Insurance_phase_to_bq) 
        print(Insurance_phase_to_bq)                                   
     
    except Exception as e:
        Subject = 'Insurance_phase_to_bq ERROR' 
