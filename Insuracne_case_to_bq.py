...
queries = {"Insurance_case_to_bq": '''select * from insurance_case
'''
...
def upload_to_BQ(agent):
    ...
    table_ref = client.dataset("tables_crons").table("Insurance_case")
    
    job_config = bigquery.LoadJobConfig(
        schema=[
            bigquery.SchemaField('id', 'INTEGER'),
            bigquery.SchemaField('patientId', 'INTEGER'),
            bigquery.SchemaField('facilityId', 'INTEGER'),
            bigquery.SchemaField('vendorId', 'INTEGER'),
            bigquery.SchemaField('status', 'STRING'),
            bigquery.SchemaField('comments', 'STRING'),
            bigquery.SchemaField('personnelId', 'INTEGER'),
            bigquery.SchemaField('treatmentType', 'STRING'),
            bigquery.SchemaField('surgeryId', 'INTEGER'), 
            bigquery.SchemaField('surgeryName', 'STRING'),
            ...
            bigquery.SchemaField('currentlyAssignedTo', 'STRING'),
            bigquery.SchemaField('pendingPhaseType', 'STRING'),
...
def lambda_handler(event, context): 
    ...
    try:
        ...
        Insurance_case_to_bq = pd.DataFrame(dataSets['Insurance_case_to_bq'])
 
        upload_to_BQ(Insurance_case_to_bq) 
        # print(Insurance_case_to_bq)
     
    except Exception as e:
        Subject = 'Insurance_case_to_bq ERROR' 
